...
if(parameter %% 3 == 0){
	...
	PERCENT_HET_THRESHOLD = 0.2
}else{
	PERCENT_HET_THRESHOLD = 0.3
}

#NUM_MARKERS = as.numeric(args[1])
#PERCENT_HET_THRESHOLD = as.numeric(args[2])
print(NUM_MARKERS)
print(PERCENT_HET_THRESHOLD)
	...
	#        print(i)
			...
			if(N1s[j,i]/NON_MISSING[[CHR]][j,i] < PERCENT_HET_THRESHOLD){
				if(S != -9){
					...
					starts = c(starts, S)
					ends = c(ends, E-1)
					indices_s = c(indices_s, (S*NUM_MARKERS)+1)
					indices_e = c(indices_e, (E*NUM_MARKERS)+NUM_MARKERS)
					S = -9
					E = -9
				}else{
					donothing = 1
		...
		if((length(indices_s) > 0) && (indices_e[length(indices_e)] > nrow(map))){
...
#		segmentlengths[[j]] = c(segmentlengths[[j]], positions_e - positions_s)
	...
	write.table(towrite_bed, file=paste("/scratch/junzli_flux/sramdas/haplotyping/founders/rn6/sensitivityanalysis/strain_",strains[j],"highhetsegments_",NUM_MARKERS,"_",PERCENT_HET_THRESHOLD,".bed",sep=""),row.names=FALSE,col.names=FALSE,quote=FALSE,sep="\t")
}
